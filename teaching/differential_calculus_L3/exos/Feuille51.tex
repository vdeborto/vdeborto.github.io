...
\leftcentersright[0]{{\bf\sf\large Formation en Mathématiques Commune Cachan/P7}}{}
    ...
    \vskip3ex\centers{{\bf\sf \Large Feuille 5 - Convexité, optimisation}}
...
\vskip2ex
\begin{exo}[Caractérisation de la convexité]

\begin{questions}

\item On suppose que $f$ est différentiable sur $U$ convexe. Montrer que $f$ est convexe si et seulement si 
...
\item Exhiber un exemple de fonction $f : \RR^2 \ \mapsto \RR$ telle que $\forall x \in \RR$, $f(x,\cdot)$ est convexe et $\forall y \in \RR$, $f(\cdot,y)$ est convexe mais $f$ n'est pas convexe.
\end{questions}
\end{exo}


\vskip2ex \begin{exo}[Moindres carrés]
...
\vskip2ex \begin{exo}[Moyenne de Fréchet]
...
\vskip2ex \begin{exo}[Norme et majoration]

On considère $E$ égal à l'ensemble $\RR^d$ privé de $\{ x \in \RR^d \quad | \quad \exists i : \quad x_i = 0 \}$.  On note $N_1 (x) = \sum_i |x_i|$ et $N_2 (x ) = \sqrt{ \sum_i x_i^2}$. Soit $C>0$.
\begin{questions}
...
\vskip2ex \begin{exo}[Une caractérisation des matrices spéciales orthogonales]
...
\vskip2ex \begin{exo}[Loi de Descartes]
...
% \vskip2ex \begin{exo}[Descente de gradient]
...
% $$ f (x^k - \rho \nabla f(x^k)) = \underset{ \rho \in \RR^+}{\min} f(x^k - \rho \nabla f(x^k) ) $$

% Vérifier que si $a$ est une solution du problème de minimisation, $\alpha \Vert x^k - a \Vert \leq \Vert \nabla f(x^k) \Vert$.
% \item Montrer que $< \nabla f(x^{k+1}), \nabla f(x^k)> = 0$ pour tout $k$. En déduire que la suite $f(x^k)$ est une suite décroissante et que $ \underset{k \longrightarrow \infty}{\lim} \Vert x^{k+1} - x^k \Vert = 0$.
...
% \vskip2ex \begin{exo}[Méthode de Newton]
...
% \item Faire le lien avec l'exercice précédent.
% \end{questions}
