...
\vskip3ex\centers{{\bf\sf \Large Feuille 4.1 - Sous-variétés, introduction}}
...
\vskip2ex
\begin{exo}[Quelques exemples (1)]
Les dessins suivants représentent des parties de $\RR ^2$ ou $\RR ^3$. Dites, sans justification rigoureuse, lesquelles sont des sous-variétés $C ^\infty$.

%\begin{figure}[H]
\includegraphics[width=400px]{exemples.png}
%\end{figure}
\end{exo}
...
Soit $U$ un ouvert de $\RR ^n$ et soit $f:U\to\RR^p$ une application de classe $C^r$.\\
On dit que $f$  est une \textbf{immersion} en $x\in U$ si sa différentielle en $x$ est injective (i.e. si sa matrice jacobienne est de rang $n$).\\
On dit que $f$ est une \textbf{submersion} en $x \in U$ si sa différentielle en $x$ est surjective (i.e. si sa matrice jacobienne est de rang $p$). $f$ est une\textbf{ submersion} au dessus de $y\in \RR^p$ si $f$ est une submersion en tout point de $f^{-1}(y)$.


Soit $U$ un ouvert de $\RR^n$. Soit $f:U\to\RR^p$ de classe $C^r$.
\begin{questions}
\item Montrer que si $f$ est une immersion en $x\in U$, alors il existe un ouvert $x\in U'\subset U$ tel que $f(U')$ est une sous-variété de classe $C^r$ et de dimension $n$ de $\RR^p$.
\item montrer que si $f$ est une submersion au dessus de $y\in \RR^p$, alors $f^{-1}(y)$ est ou bien vide, ou bien une sous-variété de classe $C^r$ et de dimension $n-p$ de $\RR^n$.
\end{questions}

\end{exo}
}
\vskip2ex
\begin{exo}[Quelques exemples (2)]
Les sous-ensembles $V$ de $\RR^2$ ou $\RR^3$ qui suivent sont-ils des sous-variétés ? Et $V \setminus \{0 \}$ ?

\begin{questions}
\item $ V = \RR ^2$

\item $V=\{(t,0) \in \RR^{2} / \ t \in [0,1[ \}$;

\item $V = \{(x,y) \in \RR^2, \quad y = |x| \}$. 

\item $V = \{(x,y,z) \in \RR^3, \quad x^2 + y^2 = z^2 \}$;
...
\vskip2ex
\begin{exo}[Le cas du tore]
Montrer que $ E = \{(x,y) \in \RR^2, \quad x^2 + 2y^2 = 1 \}$ est une sous-variété de $\RR^2$. Montrer que $ E ' = \{(x,y,z) \in \RR^3, \quad x^2 + y^2 +2 z^2 = 1 \}$ est une sous-variété de $\RR^{3}$. Montrer qu'un tore dans $\RR^{3}$ est une sous-variété de $\RR^{3}$ (on pourra considérer le cas du tore symétrique par rapport à l'axe $z$, d'équation $(R - \sqrt{x^2 + y^2})^2 + z^2 = r^2$).

\end{exo}


\vskip2ex
\begin{exo}[Inversibilité et sous-variété]
\begin{questions}
\item Montrer que l'ensemble $A = \{ (x , y) \in \RR ^2 | x=0 \texttt{ et }  0 \leq y < \frac{1}{2}, \texttt{ ou } 0 \leq 0 < \frac{1}{2} \texttt{ et } y = 0 \}$ n'est pas une sous-variété $C^\infty$ de $\RR ^2$. 

\item  Donner cependant un exemple d'application $C ^\infty$ d'un intervalle de $\RR$ dans $\RR ^2$ d'image $A$.

\end{questions}
\end{exo}
\vskip2ex
\begin{exo}[Lignes de niveaux et sous-variétés]
Montrer que les lignes de niveau de la fonction suivante sont des sous-variétés $\RR^3$: 
$$ F(x,y,z) = \frac{x^2 + y^2}{x^2 + y^2 + z^2 + (1 - x^2 - y^2)^2} $$
...
\item Montrer que l'ensemble des matrices symétriques de rang $r$ forment une sous-variété. Donner sa dimension.
\poubelle{
