...
\begin{center}
\huge{TD 9 \\ Équations différentielles}
\end{center}


\begin{exo}
\begin{questions}
\item  \textbf{Critère de Cauchy pour la limite d'une fonction en un point} Soit $f : U \subset \RR ^n \longrightarrow \RR ^p$, soit $ a \in \overline{U}$. Montrer que $f$ admet une limite en $a$ $ssi$ $\forall \epsilon$, $\exists \eta >0$ tel que $\forall x , y \in U$, si $|x-a| < \eta$ et $|y-a| < \eta$ , alors $|f(x) - f(y)| < \epsilon$.
...
Montrer que $\omega (x)$ est stable par le flot de $f$ $ie$ que $\forall y \in \omega(x)$ et $\forall t \in J_x $ , $\phi_t (y) \in \omega (x)$.
\end{exo}
