...
deux fois différentiable. Montrer que l'application $x \mapsto \der f_{x}(x)$ est différentiable et calculer sa différentielle.
...
\begin{exo} Soient $U$ un ouvert convexe d'un espace vectoriel normé $E$ et $f : U \ra \RR$ une application. On dit que 
...
\newpage
\vskip2ex
\begin{exo}[Application : dérivations de $C^{\infty}(\RR)$]
Soit $C^{\infty}(\RR)$ l'algèbre des fonctions de $\RR$ dans $\RR$ de classe $C^{\infty}$. On appelle dérivation de $C^{\infty}(\RR)$ tout endomorphisme linéaire $\delta$ de $C^{\infty}(\RR)$ tel que :
\begin{equation*}
 \forall (f,g) \in C^{\infty}(\RR), \ \delta(fg)=f \delta(g) + g \delta(f)
\end{equation*}
\begin{questions}
\item Soit $\delta$ une dérivation. Montrer que $\delta$ est nulle sur les fonctions constantes.
\item Montrer que si $f,g \in C^{\infty}(\RR)$ et s'il existe $x_{0} \in \RR$ tel que $f'(x_{0})=g'(x_{0})$ alors $\delta(f)(x_{0})=\delta(g)(x_{0})$.
\item En conclure qu'il existe $u\in C^{\infty}(\RR)$ telle que $\delta : \ f\mapsto u f'$.
%\item Comment généraliser à l'algèbre $C^{\infty}(\RR^{n})$ des fonctions de $\RR^{n}$ dans $\RR$ ?
\end{questions}
\end{exo}
