...
for nimg = 1:Nimg
    tic;
    nimg
    % on se trouve dans res/res_internal_percep/
    nimg_s = ind2str(nimg,Nimg);
    mkdir(nimg_s)
    cd(nimg_s)
    ...
    for n = 1:N
        ...
        if L_h>1
            h = h - mean(h(:));
            h = h * std(im(:)) / (std(h(:))*L_h); % doing so we ensure ourselves
                                                  % that we have the good order
        ...
        for n2 = 1:length(spotsize_t)
            for n3 = 1:length(method_c)
                ...
                for n4 = 1:length(nfa_t)
                    ...
                    percepthres = draw_patch(percepthres,1,1,p,'draw', ...
                                             'green');
                    percepthresmin = draw_patch(percepthresmin,1,1,p,'draw','green');
    ...
    % on revient dans res/res_internal_percep
    cd ..
end